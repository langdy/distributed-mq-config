...
public class DefaultInjectionAttackHandler implements InjectionAttackHandler {
    ...
    public boolean isSqlInjectionAttack(String rawCharacters, String[] ignoreStrings) {
        ...
        return this.ignoreStringMatchingHandle(this.getSqlInjectionAttackMatcher(rawCharacters),
                ignoreStrings);
    }
    ...
    public boolean isXSSInjectionAttack(String rawCharacters, String[] ignoreStrings) {
        ...
        return this.ignoreStringMatchingHandle(this.getXSSInjectionAttackMatcher(rawCharacters),
                ignoreStrings);
    }
    ...
    public boolean isSpecialCharactersInjectionAttack(String rawCharacters,
        ...
        return this.ignoreStringMatchingHandle(
                this.getSpecialCharactersInjectionAttackMatcher(rawCharacters), ignoreStrings);
    }

    @Override
    public String filter(String rawCharacters) {
        return this.filterSpecialCharacters(
                this.filterXSSInjection(this.filterSqlInjection(rawCharacters)));
    }
    ...
    public void attackHandle(HttpServletRequest request, HttpServletResponse response,
        ...
        try (PrintWriter out = response.getWriter()) {
            out.print(JsonUtils.toCustomizationJson(
                    new ResponseEntity<Object>().badRequest("请求内容包含非法字符,原请求内容:\n" + parameters)));
        }
    }

    private static class SingletonDefaultInjectionAttackHandler {
        private static final DefaultInjectionAttackHandler INSTANCE =
                new DefaultInjectionAttackHandler();
    }
