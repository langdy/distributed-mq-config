...
/**
 * 获取上下文中所有Mapping映射地址信息,默认不开启
 * <p>
 * application.yml 配资示例
 * 
 * <pre>
 *     distributed:
 *       basic-bean:
 ...
 */
@Configuration
@ConditionalOnExpression("${distributed.basic-bean.enabled:false}")
public class BasicBeanConfig implements ApplicationContextAware, ApplicationListener<ContextRefreshedEvent> {
	...
	/**
	 * [{[/authentication]=[POST]}, {[/authentication]=[PUT]}, {[/captcha/login]=[GET]}, {[/captcha]=[GET]},
	 * {[/api/user/{id}]=[GET]}, {[/api/user/{id}]=[PUT]}, {[/api/user/{id}]=[DELETE]}, {[/api/user]=[POST]},
	 * {[/api/user/test]=[GET]}, {[/api/user]=[GET]}, {[/api/user/distributed-lock]=[GET]}, {[/api/user/async]=[GET]},
	 * {[/api/user/list]=[GET]}, {[/v2/api-docs]=[GET]}, {[/swagger-resources/configuration/security]=[]},
	 * {[/swagger-resources/configuration/ui]=[]}, {[/swagger-resources]=[]}, {[/error]=[]}, {[/error]=[]}]
	 */
	...
	private Set<RequestMethod> requestMethodsRequestConditionAll = new LinkedHashSet<>(
			Arrays.asList(RequestMethod.GET, RequestMethod.HEAD, RequestMethod.POST, RequestMethod.PUT,
	...
	public void onApplicationEvent(ContextRefreshedEvent event) {
		...
		handlerMethods.keySet().forEach(mappingInfo -> {
			Map<Set<String>, Set<RequestMethod>> mapping =
					Collections.singletonMap(mappingInfo.getPatternsCondition().getPatterns(),
							this.getMethods(mappingInfo.getMethodsCondition().getMethods()));
			requestMappingInfos.add(mapping);
		});

		requestMappingUris.addAll(handlerMethods.keySet().parallelStream()
